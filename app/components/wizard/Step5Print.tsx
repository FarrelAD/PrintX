  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="space-y-6">
          <div className="p-6 bg-white border border-primary space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                  {t('wizard.step5.form.paper_size')}
                </label>
                <select
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                  {t('wizard.step5.form.orientation')}
                </label>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-outline-variant relative">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                  {t('wizard.step5.form.design_width') || 'Lebar Desain (cm)'}
                </label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                  {t('wizard.step5.form.design_height') || 'Tinggi Desain (cm)'}
                </label>
                <input
                  type="number"
                  step="0.1"
              ...
              <button
                ...
                title={
                  lockAspectRatio
                    ? t('wizard.step5.lock_ratio') || 'Kunci Rasio Aspek'
                    : t('wizard.step5.unlock_ratio') || 'Rasio Aspek Bebas'
                }
              >
            ...
            <div className="flex flex-col gap-1.5 pt-4 border-t border-outline-variant">
              <label className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                {t('wizard.step5.form.bleed') || 'Bleed / Margin Potong (mm)'}
              </label>
              <input
                type="range"
                min="0"
                ...
                onChange={(e) =>
                  setConfig({ ...config, bleedMm: parseInt(e.target.value) })
                }
            ...
            <div className="flex items-center gap-6 pt-4 border-t border-outline-variant">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, showCropMarks: e.target.checked })
                  }
                  className="w-4 h-4 accent-primary"
                />
                <span className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                  {t('wizard.step5.form.crop_marks') ||
                    'Tanda Potong (Crop Marks)'}
                </span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  checked={config.nUp}
                  onChange={(e) =>
                    setConfig({ ...config, nUp: e.target.checked })
                  }
                  className="w-4 h-4 accent-primary"
                />
                <span className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                  {t('wizard.step5.form.n_up') || 'Optimasi Layout (N-Up)'}
                </span>
              </label>
            </div>
            ...
            {config.nUp && (
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-outline-variant animate-in fade-in slide-in-from-top-2">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-secondary text-left">
                    {t('wizard.step5.form.horizontal_gap') ||
                      'Gap Horizontal (mm)'}
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        gapHorizontalMm: parseInt(e.target.value) || 0,
                      })
                    }
                    placeholder="0"
                    className="p-2.5 border-2 border-primary text-sm font-bold focus:outline-none"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-secondary text-left">
                    {t('wizard.step5.form.vertical_gap') || 'Gap Vertikal (mm)'}
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        gapVerticalMm: parseInt(e.target.value) || 0,
                      })
                    }
                    placeholder="0"
                    className="p-2.5 border-2 border-primary text-sm font-bold focus:outline-none"
        ...
        <div className="flex flex-col gap-4">
          <div className="flex-1 bg-surface-container-low border border-outline-variant p-8 flex items-center justify-center relative min-h-[300px]">
            ...
            >
              <div className="grid grid-cols-2 gap-1 opacity-20">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div
                    key={i}
                    className="border border-primary w-20 h-12 bg-primary/5 flex items-center justify-center text-[6px]"
                  >
                    DESIGN
                  </div>
                ))}
            ...
            <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center text-[9px] font-bold uppercase tracking-widest text-secondary">
              <span>
                {t('wizard.step5.preview_label') || 'Preview Layout Kertas'}
              </span>
              <span>
                {config.paperSize}{' '}
          ...
          <div className="p-4 bg-primary text-white space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">
              {t('wizard.step5.result_estimate') || 'Estimasi Hasil'}
            </p>
            <p className="text-xl font-heading">
              {data.dataset?.rows.length ?? 0}{' '}
              {t('wizard.step5.documents_label') || 'Dokumen'} • 300 DPI • CMYK
              Ready
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4">
        ...
        >
          {isGenerating
            ? t('wizard.step5.actions.processing') || 'Memproses...'
            : t('wizard.step5.actions.generate') || 'Download PDF Siap Cetak'}
        </button>
      </div>
    </div>
  );
