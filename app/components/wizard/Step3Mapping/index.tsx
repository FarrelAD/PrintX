...
export default function Step3Mapping({
  ...
  const [showDesign, setShowDesign] = useState(
    data.editorSettings?.showDesign ?? true,
  );
  ...
  const updateField = useCallback(
    (id: string, patch: Partial<MappingField>) => {
      setFields((prev) =>
  ...
  const handleClearAll = useCallback(() => {
    if (
      window.confirm(
        t('wizard.step3.reset_confirm') ||
          'Hapus semua bidang yang sudah ditempatkan?',
      )
    ) {
      setFields([]);
      setSelectedId(null);
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-4">
        ...
        <div className="w-full lg:w-56 shrink-0 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex lg:flex-col gap-1.5 max-h-[300px] lg:max-h-[420px] overflow-y-auto pr-1">
              {headers.map((header) => (
                <ColumnChip
                  ...
                  onClick={() =>
                    setActiveColumn((prev) => (prev === header ? null : header))
                  }
                />
              ))}
            </div>
          ...
          <div className="mt-2 p-3 bg-surface-container border border-outline-variant text-[9px] text-secondary leading-relaxed">
            ...
            {activeColumn
              ? t('wizard.step3.active_hint', { column: activeColumn }) ||
        ...
        <div className="flex-1 flex flex-col gap-2 min-w-0">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold uppercase transition-colors ${
                    showDesign
                      ...
                      : 'bg-white text-secondary border border-outline-variant'
                  }`}
                >
                  <span className="material-symbols-outlined text-[14px]">
                  ...
                  {showDesign
                    ? t('wizard.step3.design_on') || 'Desain On'
                    : t('wizard.step3.design_off') || 'Desain Off'}
                </button>
              </div>
              <div className="h-4 w-px bg-outline-variant" />
              <div className="flex items-center gap-3">
                <span className="text-[9px] font-bold uppercase text-secondary">
                  {t('wizard.step3.canvas_bg') || 'BG Kanvas'}:
                </span>
                <div className="flex gap-1.5">
                  {[
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor:
                          c.value === 'transparent' ? '#fff' : c.value,
                        backgroundImage:
                          c.value === 'transparent'
              ...
              {!data.design?.preview && (
                <div className="absolute inset-0 flex items-center justify-center flex-col gap-2 text-secondary bg-surface-container/50">
                  ...
                  <span className="text-xs uppercase tracking-widest">
                    {t('wizard.step3.no_design_hint') ||
                      'Tidak ada desain dimuat'}
                  </span>
                </div>
              )}
              ...
              {activeColumn && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 bg-secondary/10">
                  <div className="bg-secondary text-white text-[10px] font-bold uppercase tracking-widest px-4 py-2 shadow-lg flex items-center gap-2">
                    ...
                    {t('wizard.step3.click_hint', { column: activeColumn }) ||
          ...
          {data.design?.preview && fields.length === 0 && (
            <div className="flex items-center gap-2 text-[10px] text-secondary italic mt-1">
              ...
              {t('wizard.step3.empty_canvas_hint') ||
      ...
      <div className="mt-10 flex justify-between gap-4">
        ...
        <button
          ...
          title={
            !hasMappings
              ? t('wizard.step3.mapping_required') ||
                'Tambahkan minimal satu bidang ke kanvas terlebih dahulu'
              : ''
          }
