...
export default function StatusDropdown({
  ...
  const statuses: ProjectData['status'][] = [
    'draft',
    'in_progress',
    'ready',
    'finished',
  ...
  const getStatusColor = (status: ProjectData['status']) => {
    ...
    switch (key) {
      ...
      case 'in_progress':
        return 'bg-surface-container-high text-primary border-primary/10';
      default:
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`w-1.5 h-1.5 rounded-full ${getStatusDot(currentStatus)}`}
        />
        {t(`status.${currentKey}`)}
        <span
          className={`material-symbols-outlined text-[14px] transition-transform duration-200 ${isOpen ? (direction === 'up' ? 'rotate-0' : 'rotate-180') : direction === 'up' ? 'rotate-180' : 'rotate-0'}`}
        >
          expand_more
        </span>
      </button>
      ...
      {isOpen && (
        <div
          className={`absolute ${direction === 'up' ? 'bottom-full mb-1' : 'top-full mt-1'} left-0 w-48 bg-white border border-primary shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-110 animate-in fade-in ${direction === 'up' ? 'slide-in-from-bottom-2' : 'slide-in-from-top-2'} duration-150`}
        >
          ...
          <div className="py-1">
            {statuses
              .filter((s) =>
                ['draft', 'in_progress', 'ready', 'finished'].includes(
                  s as string,
                ),
              )
              .map((status) => (
