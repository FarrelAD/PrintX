...
export default function DashboardHome() {
  ...
  const activeProjects = validProjects.filter(
    (p) => !p.status || p.status === 'draft' || p.status === 'in_progress',
  ).length;
  const readyProjects = validProjects.filter(
    (p) => p.status === 'ready',
  ).length;
  ...
  const confirmNewProject = (name: string) => {
    ...
    const url = name.trim()
      ? `/dashboard/project/new?name=${encodeURIComponent(name.trim())}`
  ...
  return (
    <>
      ...
      <NameModal
        ...
        placeholder={
          t('common.project_name_placeholder') || 'Nama proyek Anda...'
        }
      />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-[10px] uppercase font-bold tracking-[0.2em] bg-primary text-white px-2 py-0.5">
                Vol. 01
              </span>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
        <div className="border border-primary p-6 bg-surface-container">
          <span className="text-2xl material-symbols-outlined text-secondary mb-2">
            work_history
          </span>
          <div className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-1">
            {t('dashboard.stats.active_drafts')}
          </div>
        ...
        <div className="border border-primary p-6 bg-surface-container">
          ...
          <div className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-1">
            {t('dashboard.stats.ready_to_print')}
          </div>
        ...
        <div className="border border-primary p-6 bg-surface-container">
          ...
          <div className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-1">
            {t('dashboard.stats.storage_status')}
          </div>
          <div className="text-3xl font-heading">
            {t('dashboard.stats.storage_safe')}
          </div>
        </div>
        <div
          className="border border-primary p-6 bg-surface-container flex flex-col justify-between items-start cursor-pointer hover:bg-surface-container-high transition-colors"
          onClick={() => navigate('/dashboard/projects')}
        >
          <span className="text-2xl material-symbols-outlined text-primary mb-2">
            folder_open
          </span>
          <div className="text-[10px] uppercase tracking-widest text-primary font-bold mb-1">
            {t('dashboard.stats.view_all')}
          </div>
          <span className="material-symbols-outlined text-primary self-end">
            arrow_forward
          </span>
        </div>
      </section>
      ...
      <section className="mb-12">
        <div className="flex justify-between items-end mb-6 border-b border-primary/20 pb-4">
          ...
          <Link
            ...
            className="text-xs uppercase font-bold tracking-widest text-primary hover:underline"
          >
            {t('dashboard.all_projects')} &rarr;
          </Link>
        </div>
        ...
        {isLoading ? (
                ...
                >
                  <div className="aspect-4/3 bg-surface-container border border-primary relative overflow-hidden mb-4 transition-all group-hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] group-hover:-translate-x-1 group-hover:-translate-y-1">
                    {project.design?.preview ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-20 group-hover:opacity-40 transition-opacity">
                        <span className="text-[80px] material-symbols-outlined">
                          description
                        </span>
                      </div>
                    )}
                    <div className="absolute top-0 left-0 p-3">
                      <div className="text-[9px] font-mono bg-white border border-primary px-2 py-0.5">
                        ID: {project.id?.slice(0, 8)}
                      </div>
                    </div>
                    <div
                      className={`absolute bottom-3 right-3 py-1 px-3 text-[9px] uppercase font-bold tracking-wider border border-primary shadow-sm ${
                        statusKey === 'ready'
                  ...
                  <div className="flex flex-col">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="text-xl font-heading leading-tight group-hover:italic transition-all truncate pr-4">
                        {project.name ||
                          t('projects.untitled') ||
                          'Proyek Tanpa Nama'}
                      </h3>
          ...
          <div className="border border-dashed border-outline-variant py-16 flex flex-col items-center justify-center text-secondary">
            ...
            <Link
              ...
              className="mt-4 text-xs font-bold text-primary uppercase border-b border-primary pb-0.5"
            >
              {t('common.create_now') || 'Buat Sekarang'}
            </Link>
          </div>
        )}
      </section>
