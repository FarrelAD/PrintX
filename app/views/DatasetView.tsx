...
export default function DatasetView() {
  ...
  const projectsWithDataset = projects.filter(
    (p) => p.dataset && p.dataset.rows.length > 0,
  );
          ...
          <>
            ...
            <div className="hidden md:block overflow-x-auto w-full">
              <table className="w-full text-left border-collapse">
                <thead className="bg-surface-container border-b border-primary">
                  <tr>
                    <th className="p-4 text-xs font-bold uppercase tracking-widest border-r border-primary/20">
                      {t('datasets.table.name')}
                    </th>
                    <th className="p-4 text-xs font-bold uppercase tracking-widest border-r border-primary/20">
                      {t('datasets.table.columns')}
                    </th>
                    <th className="p-4 text-xs font-bold uppercase tracking-widest border-r border-primary/20">
                      {t('datasets.table.total_rows')}
                    </th>
                    <th className="p-4 text-xs font-bold uppercase tracking-widest border-r border-primary/20">
                      {t('datasets.table.project')}
                    </th>
                    <th className="p-4 text-xs font-bold uppercase tracking-widest">
                      {t('datasets.table.action')}
                    </th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className="md:hidden flex flex-col">
              {projectsWithDataset.map((project) => (
                <div
                  key={project.id}
                  className="border-b border-primary p-4 last:border-b-0"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="bg-primary text-white px-2 py-1 text-[10px] font-mono">
                      {t('datasets.table.rows_count', {
                        count: project.dataset?.rows.length,
                      })}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-secondary block mb-1">
                        {t('datasets.table.columns_label')}
                      </span>
                      <div className="flex flex-wrap gap-1">
                        {project.dataset?.headers
                          .slice(0, 5)
                          .map((header, idx) => (
                            <span
                              key={idx}
                              className="text-[10px] bg-surface-container px-1.5 py-0.5 border border-outline-variant"
                            >
                              {header}
                            </span>
                          ))}
                        {project.dataset?.headers &&
                          project.dataset.headers.length > 5 && (
                            <span className="text-[10px] text-secondary self-center px-1">
                              {t('datasets.table.others', {
                                count: project.dataset.headers.length - 5,
                              })}
                            </span>
                          )}
                      </div>
